Tài liệu Hướng dẫn Tích hợp Frontend - Ứng dụng Chat Realtime (Spring Boot + PostgreSQL + Redis + WebSocket)
...
REST API (HTTP/HTTPS): Xử lý các tác vụ một lần (One-off) như Đăng ký, Đăng nhập, Lấy danh sách Online ban đầu.
WebSocket (STOMP qua SockJS): Đảm nhiệm luồng dữ liệu thời gian thực, duy trì kết nối liên tục để truyền tải tin nhắn, trạng thái Online/Offline và trạng thái "Đang gõ...".
Cơ chế Bảo mật: Toàn bộ hệ thống được bảo vệ bởi JWT (JSON Web Token). Token bắt buộc phải được đính kèm trong HTTP Header (Authorization: Bearer <token>) đối với REST API. Lưu ý đối với WebSocket: Khi gọi lệnh Connect, Token có thể được truyền vào trong Headers (Authorization: Bearer <token>) HOẶC truyền qua query parameter/header tùy chỉnh tên là token (để hỗ trợ trường hợp SockJS không cho đính kèm Authorization header mặc định).
...
Xác thực (Quan trọng): Khi gọi hàm connect(), bắt buộc phải truyền JWT token. Hệ thống hỗ trợ 2 cách truyền token:
Qua header Authorization: Bearer <token>
Qua header token: <token> (được khuyến nghị khi dùng SockJS để tránh lỗi CORS).
Ví dụ code JS (sử dụng @stomp/stompjs mới nhất):
...
const stompClient = new Client({
    ...
    connectHeaders: {
        // Có thể dùng Authorization hoặc truyền trực tiếp tên biến 'token'
        token: token
    },
    debug: function (str) {
        console.log(str);
...
Lưu ý Backend mới: Frontend KHÔNG CẦN CHỦ ĐỘNG GỬI gói tin báo danh /app/chat.addUser nữa. Backend sẽ tự động phát sóng thông điệp JOIN ngay khi user mở kết nối WebSocket thành công.
...
Frontend sử dụng stompClient.publish({ destination, body }) để gửi dữ liệu. Lưu ý: Thuộc tính body phải là một chuỗi JSON (sử dụng JSON.stringify()).
...
Logic:
Nếu là chat 1-1: Bắt buộc điền trường recipient. Backend sẽ lưu vào PostgreSQL và tự động định tuyến đến kênh /user/{recipient}/queue/messages.
Nếu là chat nhóm: Bắt buộc điền trường groupId. Backend sẽ lưu vào PostgreSQL và tự động định tuyến đến kênh /topic/group/{groupId}.
...
Logic:
Tương tự như gửi tin nhắn, điền recipient hoặc groupId để chỉ định đích đến của tín hiệu typing. Đầu bên kia sẽ nhận được thông điệp này và hiển thị hiệu ứng. Tín hiệu này KHÔNG lưu vào Database.
IV. Hướng dẫn và Mẹo xử lý UX/UI (Best Practices)
Quản lý Danh sách Online (Real-time):
...
Nếu nhận được message.type === "LEAVE": Lọc (filter) và xóa message.sender ra khỏi mảng onlineUsers (Tắt chấm xanh). Đồng thời Backend sẽ tự động lưu thời gian "Last Seen" của user đó vào Database.
Xử lý Hiệu ứng "Đang gõ..." (Typing Indicator):
